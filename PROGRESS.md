...
### 整合篩選分頁(已完成,2026-09-22)

**背景**:使用者搞清楚外資選擇權/大額交易人這兩個新訊號是大盤層級、不是選股工具之後,
接著問「怎麼看出要買哪隻個股期貨」——答案是既有的 💪RS排行 + 🎯做多訊號/🚦條件燈號,
但這幾個分頁是分開的,要手動對照,使用者反映「不知道要怎麼篩選」,要求整合成一個畫面。

**決定**:新增 `combined_screen.py`,只做「合併既有結果」,不新增任何判斷邏輯/門檻/
統計假設——`run_combined_screen()` 對同一份 `.dsl` 個股期貨自選股池分別呼叫既有的
`relative_strength.compute_rs_ranking()`(RS Rating)跟 `signals.scan_light_signals()`
(7條件達成燈號,兩者都已經是批次版本,效能沒有問題),用代號合併成一張表;`get_market_tone()`
把外資選擇權/大額交易人兩個大盤濾網的最新讀數包成一個dict,純粹當畫面上的環境提示,
**故意不用來過濾/排序逐股表格**(這兩個訊號還有「回測剛好在多頭期間」的但書,沒驗證過
拿來當逐股篩選條件會不會有效,不能無憑無據當篩選規則)。

**app.py**:新增分頁「⭐ 整合篩選」(在🎯做多訊號跟🔎分點掃描中間)——上面顯示兩個大盤濾網
badge,下面是可調的 RS Rating / 燈號達成數門檻滑桿 + 合併表格 + 每檔股票可展開看7個條件細節。
按鈕觸發、結果存 session_state 的模式沿用🚦/💪兩個分頁已經驗證過的既有慣例(避免
auto_refresh下一次整頁重跑把結果沖掉這個坑)。用 Playwright 在這個網路被擋的sandbox裡
實測過分頁能正常渲染(市場濾網顯示「尚未累積資料」,沒有crash);`run_combined_screen()`
也直接在Python裡測過「沒有.dsl檔案」時會乾淨回傳`{"mode": "no_watchlist"}`,不會拋例外。

**刻意沒做的**:沒有把 🔎分點掃描(`scan_known_branch_buying`)也合併進這張表——那個掃描
是掃全部 `xq_branch_data/` 檔案(不限個股期貨標的),跟這裡「同一個.dsl股票池」的合併邏輯
不完全對齊,直接塞進來會讓合併規則變複雜,先不做,之後使用者覺得需要再加。
也沒有幫使用者發明「RS>=?且燈號>=?才算候選」這種新門檻——UI上是滑桿讓使用者自己調,
不是內建一個「建議清單」,避免無憑無據幫使用者做選股決策。
